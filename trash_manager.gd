...
func spawn_rand_trash(drifts_in : bool):
	...
	trash_list.add(new_trash_node)
	new_trash_node.global_position = spawn_pos
	
	# new_trash_node.setup(sprite, health, reward_value, spawn_target)
	match randi_range(1, 1):
		1: new_trash_node.setup(null, randi_range(5,7), randi_range(1,3), spawn_target)
...
func _process(delta: float) -> void:
	...
	if not spawn_queue_times.is_empty():
		...
		while (game_time_elapsed >= spawn_queue_times[0]):
